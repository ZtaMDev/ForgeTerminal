...
export function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  ...
  const createItems = useCallback((): SettingItem[] => [
    ...
    { section: "Shortcuts", id: "commandKey", label: "Command Key (e.g. ` or m)", type: "text" as const, value: config.shortcuts.commandKey, onChange: (v: unknown) => { const val = (v as string).trim(); if (val) setShortcuts({ commandKey: val.charAt(val.length - 1) }); } },
